...
public class ArchGradientPainter extends BaseTabComponent {
    private final int[] topLeftCornerX = { 0, 1, 1, 2, 3, 4, 5, 6 };
    private final int[] topLeftCornerY = { 6, 5, 4, 3, 2, 1, 1, 0 };
    ...
    private final int tabOverlap = 24;
        ...
        if( other instanceof ArchGradientPainter ){
            ...
            if( painter.isSelected() ){
                if( getOrientation().isHorizontal() ) {
                    return new Insets( 0, 10 + tabOverlap, 0, 0 );
                } else {
                    return new Insets( 10 + tabOverlap, 0, 0, 0 );
                }
            }
        }
    ...
    protected void update(){
        ...
        if( wasPreviousSelected ){
            if( horizontal ) {
                labelInsets.left += tabOverlap;
            } else {
                labelInsets.top += tabOverlap;
            }
        }
    ...
    private Polygon leftSideTop( int x, int y, int w, int h ){
        int[] xPoints = new int[ topLeftCornerX.length+2 ];
        int[] yPoints = new int[ topLeftCornerY.length+2 ];

        System.arraycopy( topLeftCornerX, 0, xPoints, 1, topLeftCornerX.length );
        System.arraycopy( topLeftCornerY, 0, yPoints, 1, topLeftCornerY.length );
